package u02.myimplementations.task9
...
object Optionals:
  ...
  object OptionalInt:
    ...
    def mapInt(optionalInt: OptionalInt)(f: Int => Int): OptionalInt = optionalInt match
      case Just(value) => Just(f(value))
      case Empty() => Empty()

    def filter(optionalInt: OptionalInt)(f: Int => Boolean): OptionalInt = optionalInt match
      case Just(value) if f(value) => Just(value)
      case _ => Empty()
